...
public class AgencyService {
    ...
    public Pair<List<User>, Long> getAllAgent(PageParams pageParams) {
        List<User> agents = agencyMapper.selectAgent(new User(), pageParams);//获取用户数据
        setImg(agents);//获取用户头像
        //todo 作用
        Long count = agencyMapper.selectAgentCount(new User());//获取总记录数
        //ImmutablePair不可变的左右元素对
        return ImmutablePair.of(agents, count);  //返回成Pair对象
    }
